...
import {
  ...
  type NormalizedCard,
} from '../../../utils/contact-card-normalization';
...
function contactFieldsFromCard(card: any): ContactMutationFields {
  ...
  return {
    ...
    emails: normalized.emails.map(toContactDetailEmail),
...
function mergePhone(
  ...
  if (!sameStringSet(baseline.features, desired.features)) {
    assignOptional(
      next,
      'features',
      mergeKnownBooleanSet(next.features, STANDARD_PHONE_FEATURE_NAMES, desired.features),
    );
  }
  return next;
}
...
function mergeResourceMetadata(
  ...
  if (!sameStringSet(baseline.contexts, desired.contexts)) {
    assignOptional(
      next,
      'contexts',
      mergeKnownBooleanSet(next.contexts, STANDARD_CONTEXT_NAMES, desired.contexts),
    );
  }
}

...
| `StaffSettingsSection.vue` | Async "Staff settings" body of the Settings dialog: Bolt colors switch, a Refresh beacons button that restarts the announcement round, code textbox until unlocked, then an on/off switch plus seed retry; requests the first-unlock celebration and seeding. |

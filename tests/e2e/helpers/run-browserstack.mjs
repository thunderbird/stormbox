/**
 * When running in E2E_BROWSERSTACK mode, our Playwright config loads env vars from
 * tests/e2e/.env.browserstack; but browserstack-node-sdk starts before Playwright config runs.
...
process.env.E2E_BROWSERSTACK = process.env.E2E_BROWSERSTACK ?? '1';
...
if (missing.length) {
  console.error(
    `BrowserStack remote e2e requires ${missing.join(', ')} in tests/e2e/.env.browserstack or the shell environment.`,
  );
  process.exit(1);
}

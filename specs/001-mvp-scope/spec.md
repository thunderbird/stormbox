...
| R-4.7 🟩 Done | Cc/Bcc input fields with autocomplete, specified and delivered by `specs/004-compose-improvements/spec.md` §2 (CS-2.1). The submission envelope covers Cc and Bcc recipients: it is omitted on create so the server derives `rcptTo` from all three header fields per RFC 8621 §7 (CS-1.1). |

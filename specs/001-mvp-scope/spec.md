...
| 2 | Read mail | 10 | 2 | 1 | Conversation UI, full mail search, browser navigation |
| 3 | Triage | 15 | 1 | 1 | Bulk-operation dispatch contract; undo/redo queue |
| 4 | Compose and send | 6 | 2 | — | Sent reconciliation and failure UX |
| 5 | Contacts | 6 | 1 | — | Accepted unknown-address-book sync residue |
| 6 | Attachments | 1 | — | 5 | Preview, download, compose, bar layout |
| 7 | Account storage usage | 2 | — | — | — |
| 8 | Application chrome and cross-product navigation | 7 | — | 1 | services-ui adoption |
...
| R-1.3 🟩 Done | When an OIDC session is already valid on app load, the system shall reconnect without prompting the user. While connected, OIDC access-token rotations shall be propagated in memory to the active sync transport; bearer credentials shall not be persisted in application SQLite. |
...
| R-2.9 🟩 Done | The system shall let the user collapse and restore the folder list from the spaces toolbar. When the viewport is narrower than 1024 px and the message detail pane is visible, the folder list shall collapse automatically to preserve reading space. In this compact reading layout, automatic folder-list visibility shall mirror message-view visibility: the folder list collapses when the message detail appears, and restores when that pane is hidden. The toolbar button shall only toggle the current folder-list visibility state, not enable or disable the responsive behavior. Folder-list visibility changes shall use a short sliding transition unless the user requests reduced motion; when opening a message triggers the compact folder-list collapse, the message detail shall appear near the end of that transition. The compose affordance in the folder list shall remain usable at the minimum supported folder-list width. |
...
| R-3.6 🟩 Done | The system shall provide multi-select where a checkbox toggles a row's selection independently of the focused/preview row, and shift-click extends a range from an anchor. While at least one row is selected, the message view (message detail pane) shall be hidden entirely and the bulk actions shall be presented in the message list header, leading (adjacent to the select-all checkbox, with a small gap) and replacing the message-list filter buttons of R-2.8 so filters cannot be toggled during a multi-select; this works identically in all layouts including the single-mail-column layout (R-10.3), where the message list remains visible throughout. When the user shift-selects without an existing anchor, the selection range shall start at the first message visible at the top of the current message-list viewport. When messages are multi-selected, a plain click on a message row body shall clear the multi-selection and open that message for viewing. When the user holds Shift or Control while clicking anywhere on a message-list row, the click shall be handled as the matching multi-select action rather than as a view/open action. When the select-all checkbox is indeterminate because some but not all messages are selected, activating it shall clear the selection. |
| R-3.7 🟩 Done | The system shall implement Thunderbird-compatible keyboard shortcuts for compose, reply, reply-all, forward, delete, permanent delete, archive, mark read/unread, list navigation (next/prev, next/prev unread, home/end), select all, and clear selection. Shortcut displays, tooltips, and accessibility metadata shall advertise the current operating system's primary modifier: Ctrl on Windows/Linux and Command on macOS. |
...
| R-3.14 🟨 Partial | When the user issues a move or delete (move-to-Trash or permanent destroy) covering more than a configurable batch size (default 200 messages), the system shall split the dispatch into sequential JMAP `Email/set` chunks no larger than that batch size, display a modal progress indicator that names the operation and shows messages-completed / total, and block other user input until the operation finishes. Each chunk shall be its own `pending_mutations` row so the outbox apply step still keeps the local cache authoritative per chunk. On a chunk failure the system shall stop further chunks, leave already-succeeded chunks applied, surface an error that distinguishes the partial outcome (e.g. "Could not move message (requestTooLarge) (400 of 536 succeeded).") derived from the JMAP method-level error type when the server returns one, and clear the progress indicator. The current implementation instead queues one semantic mutation row, lets the JMAP backend chunk it by the live `maxObjectsInSet`, and shows the blocking overlay only above 500 messages. Confirmed wire chunks are applied transactionally and partial outcomes are surfaced, but the per-chunk durable rows, 200-message default, and stop-after-first-failed-chunk contract are not implemented. |
...
| R-3.16 🟩 Done | When the user opens a message in the Junk folder, the system shall offer a "Not junk" action that whitelists the sender and rescues the message; the same action shall be offered for a multi-selected batch in the Junk folder, whitelisting each unique sender once (de-duplicated by address) and rescuing every selected message. Whitelisting shall add each sender's address to a dedicated "Trusted senders" address book as a `ContactCard` so the server's contact trust delivers future authenticated mail from that address to the Inbox; rescuing shall clear the `$junk` keyword, set `$notjunk`, and move the affected messages from Junk to the Inbox. The action shall be offered only from the Junk folder. The system shall confirm success only when the senders were actually trusted; when the messages moved but the trust write did not apply, the system shall surface that partial outcome rather than reporting that the senders were whitelisted. Every unique To, Cc, and Bcc address on a confirmed outgoing send shall enter the same account-wide de-duplicated trusted-contact path, including newly observed Sent Emails authored by the user in another client; rejected or ambiguous sends and received mail shall not create contacts. |
| R-3.17 🟩 Done | When the user is outside the Junk folder, the system shall offer a "Junk" action on the open-message toolbar and in the multi-select bulk actions (R-3.6) that marks the affected messages as junk: it shall set the `$junk` keyword, clear `$notjunk`, and move the messages to the account's Junk folder, applying the keyword change optimistically in the local cache and queueing one batched keyword mutation plus the move through the mutation outbox. Marking as junk shall not modify the user's contacts or trusted senders. The action shall not be offered inside the Junk folder (where "Not junk" of R-3.16 takes its place), and the system shall surface an error when no Junk folder is configured. A transient success notice shall confirm how many messages were marked as junk. |
...
| R-4.2 🟩 Done | The system shall offer rich-text editing via one shared Squire component and send a plain-text alternative alongside the HTML body. Compose bodies and Identity signatures reuse the component specified by `specs/005-contact-details/spec.md` CT-5.1 through CT-5.2. |
| R-4.3 🟩 Done | The system shall let the user pick a sending identity when more than one is available and designate one as Primary. New compose windows shall use the client-selected Primary Identity; absent a valid selection, the first server-protected Identity shall be Primary, falling back to the first Identity. Replies shall use the first Identity matching the original message's To addresses and fall back to Primary. |
...
| R-4.5 🟨 Partial | When send fails, the system shall surface the failure and keep the compose draft available. Retry shall be offered only for failures proven to have occurred before submission was accepted; where the outcome is unknown the system shall present that state explicitly rather than a Retry action, because retrying an accepted submission delivers a second copy. See `specs/004-compose-improvements/spec.md` CS-1.9. |
| R-4.6 🟩 Done | Visible Reply-all and Forward toolbar buttons. |
| R-4.7 🟩 Done | Cc/Bcc input fields with autocomplete, specified and delivered by `specs/004-compose-improvements/spec.md` §2 (CS-2.1). The submission envelope covers Cc and Bcc recipients: `rcptTo` is built client-side as the de-duplicated union of all three header fields per RFC 8621 §7 (CS-1.1). |
...
| R-4.9 🟨 Partial | The system shall let the user attach regular files in compose via a paperclip control beside Send, upload them immediately through the worker, and include them in draft and sent MIME. Pasted supported raster images up to 10 MiB shall remain inline per R-4.8; every other pasted file shall use the regular attachment path. Attachment bytes shall not be stored locally; reloading before the first confirmed draft checkpoint may require re-selection. The picker, session state, immediate upload, paste classification, and draft-result merge are complete; same-account canonical blob reuse and successor/destroy sequencing remain in `specs/010-attachments/spec.md` AT-5.2 through AT-5.3. |

### 5. Contacts

| ID / Status | Requirement |
|:--|:--|
| R-5.1 🟨 Partial | When the JMAP Contacts capability is advertised, the system shall sync contacts into disposable local cache storage. A contact-schema migration may invalidate all cached contacts and normalized details while preserving AddressBooks and durable pending mutations; the next authoritative JMAP synchronization rebuilds the cache. Runtime sync shall ingest cards in the JSContact (RFC 9553) shape the server serves — an `addressBookIds` map, a keyed `emails` map, `name.full`, and an `organizations` map — and shall also tolerate the older single-book / flat-array shape (`addressBookId`, `emails: [...]`, `fullName`, `organization`), normalizing both to the local contact model. A card filed only in an address book the client does not yet know about is skipped fail-closed and prevents the authoritative sweep from deleting anything, but `ensureContacts()` does not refresh AddressBooks first; the card can remain absent locally until another ContactCard push or a restart even after the book becomes known. This residue is accepted. |
| R-5.2 🟩 Done | The system shall let the user browse and filter synced contacts and Identities in a dedicated three-column workspace. The address-book rail, shared virtualized directory list, responsive detail/editor pane, row contents, keyboard behavior, and dirty-navigation rules are specified by `specs/005-contact-details/spec.md` CT-1. |
| R-5.3 🟩 Done | The system shall offer recipient autocomplete in compose using live synced ContactCards as its only candidate source. Prior send history shall provide rebuildable recency/frequency ranking evidence and shall automatically promote eligible historical recipients to ContactCards once at bootstrap; a history row alone shall never make an address suggestible. |
| R-5.4 🟩 Done | When contact sync is unavailable, compose shall remain usable. |
| R-5.5 🟩 Done | The contacts view shall present an address-book rail that lists each synced address book by its server name with a per-book contact count plus an "All contacts" entry; selecting a book filters the contact list to that book and targets newly-created contacts to it. The account's default book shall carry a `Personal` badge, and unsubscribed books shall remain available with a muted status. Selecting "All contacts" shall target new contacts to the account's default book. |
| R-5.6 🟩 Done | The system shall let the user add, edit, and remove contacts through standard JMAP `ContactCard/set`. A contact may omit email and may carry repeated keyed emails, phones, websites, standard dates, notes, and Work affiliations. New contacts shall be filed in the selected address book, or the account's default book when viewing "All contacts". Editing shall issue sparse stable-key patches against an authoritative card so unknown properties, address-book membership, metadata, and concurrent additions survive. Each create, edit, and remove shall flow through the mutation outbox and reconcile the local cache before resolving. The complete field and preservation contract is `specs/005-contact-details/spec.md` CT-2 through CT-3. |
| R-5.7 🟩 Done | The address-book rail shall expose **Manage identities** at its bottom. Activating it shall load cached JMAP Identities into the same filtered, virtualized list and third-column shell used for contacts and refresh them in the background. The user shall be able to edit every mutable RFC 8621 Identity field: name, create-only email, Reply-To, Bcc, HTML signature, and text signature; remove remains gated by server `mayDelete`. Existing email remains read-only because Stalwart rejects it in an update. Identity writes shall use the durable outbox, targeted authoritative read-back, and stable typed failures. Identity detail, validation, compose-default, and reconciliation behavior is specified by `specs/005-contact-details/spec.md` CT-4 through CT-5. |
| R-5.8 🟩 Done | The contact list shall support contact-only multi-selection, scope-aware plural Delete, and move-only transfer between concrete AddressBooks. All Contacts deletion permanently destroys cards; deletion in a concrete AddressBook removes that membership and destroys a card only when it has no other membership. Transfer shall preserve unrelated memberships and use both drag/drop and a keyboard/touch Move menu. Selection, rights, durable batching, sparse JMAP patches, and verified-consistency behavior are specified by `specs/006-contact-list-actions/spec.md`. |
| R-5.9 🟩 Done | The contacts workspace shall support capability- and rights-gated AddressBook create, metadata edit, default changes, and warned permanent deletion. Deletion shall bypass Contacts Trash and reconcile exclusive destruction and shared-membership removal before success. The complete UI, durability, recovery, and confirmation contract is `specs/009-addressbook-management/spec.md`. |
...
| R-6.2 🟩 Done | The system shall let the user download every attachment through the authenticated worker using the message's owning account. Download-only types include SVG, HTML, XML, archives, executables, and other active or binary formats specified in `specs/010-attachments/spec.md` AT-1.7. |
| R-6.3 🟩 Done | Successfully resolved inline `cid:` images shall remain at their authored location in the message body (R-2.11). Ordinary raster attachments up to 10 MiB shall auto-preview after the authored body in MIME order, shrink to the reading width without upscaling, and remain listed in the attachment bar. Plain-text attachments shall preview only on request, escaped and capped. |
| R-6.4 🟩 Done | The attachment bar shall be a sibling of the message body scroll region: with short content it follows the body upward; with long content the body alone scrolls and the bar stays visible, using bounded internal overflow for long lists. |
| R-6.5 🟩 Done | Unused successful JMAP uploads expire server-side; the client retains the source file until a draft checkpoint confirms attachment handles and can re-upload or require re-selection on `blobNotFound`. See `specs/010-attachments/spec.md` AT-3.7 and AT-4.4. |
...
| ID / Status | Requirement |
|:--|:--|
| R-8.1 🟩 Done | The system shall present one top bar above the active Mail or Contacts space. Its left cell shall hold a Thundermail product menu, its centre cell shall hold the single Quick Filter shared by both spaces, and its right cell shall hold a dark/light theme toggle and an account avatar menu. In Mail the filter shall follow R-2.8; in Contacts the same field shall filter the active contact or identity list by name or email address. The Thundermail menu trigger shall keep consistent pill geometry between its closed and open states, and its product icon shall align with the spaces toolbar icon column. |
...
| R-8.8 🟩 Done | The HTML message viewer shall preserve the sanitized email document's author styles and presentation — including `<head><style>`, `<html>`/`<body>` classes and inline styles, and legacy body color attributes — before applying theme adaptation. In light mode the email's authored colors and backgrounds shall win. In dark mode, an email with active native dark support (`@media (prefers-color-scheme: dark)`, `color-scheme: dark`, or `filter: invert(...)` on `<html>`, `<body>`, or an outermost body wrapper) shall remain unchanged; otherwise the viewer shall remove bright or low-contrast backgrounds and text colors while retaining already-readable and adapted chromatic colors. The iframe document canvas shall remain transparent so an authored body background covers the full viewport even for short messages, while the iframe element supplies the active theme's fallback background for unstyled messages. Legacy `bgcolor`, `text`, `link`, `vlink`, and `alink` body attributes shall be honored in light mode and removed during dark adaptation. In dark mode the message toolbar shall provide a per-message light-mode escape hatch, reset when another message opens. |

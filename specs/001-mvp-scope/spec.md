...
| R-5.7 🟩 Done | The address-book rail shall expose **Manage identities** at its bottom. Activating it shall load cached JMAP Identities into the same filtered, virtualized list used for contacts and refresh the authoritative Identity snapshot in the background. The user shall be able to add an Identity with a display name and email address, edit its display name, and remove it only when its server `mayDelete` property permits it. An existing Identity's address shall remain read-only because the supported Stalwart `Identity/set` implementation rejects `email` in an update, including when its value is unchanged. Identity writes shall use `Identity/set` through the mutation outbox and shall not resolve successfully until an authoritative `Identity/get` read-back has updated the local cache. Identity action failures shall return stable typed outcomes for validation, an address not configured for the account, missing identity, permission denial, cache reconciliation failure, service unavailability, and unknown failure. The address-not-configured outcome shall tell the user they cannot send from that address and must add it to their account first; it shall not be collapsed into the generic save failure. |

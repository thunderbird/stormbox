...
|:--|:--|
| FM-1.1 🟨 Partial | The current JMAP Session shall be authoritative for account scopes rendered for the active user. Ingest shall upsert every account the Session advertises and transactionally remove cached non-primary accounts absent from the Session, including pending mutations that target their data. Revoked-share cleanup is implemented; distinguishing secondary personal accounts from shared scopes remains incomplete. Cross-login database isolation is tracked separately in issue #81. |
| FM-1.11 🟩 Implemented | A non-primary account shall not be required to advertise `urn:ietf:params:jmap:mail` to be ingested. RFC 8620 §2 requires a server to advertise a capability whenever the user may call its methods on that account, but Stalwart through 0.15.x derived a shared account's `accountCapabilities` from the shared principal's own permission set, so a group holding only the send/receive email permissions advertised no mail capability while still answering `Mailbox/get` for its members — which is why such folders appear in IMAP clients and not in JMAP ones. Stalwart 0.16.0 evaluates those capabilities against the requesting user's token and advertises correctly, so this requirement is inert against 0.16.0 and later. `Mailbox/get` shall be the arbiter of whether a shared account has mail: per FM-1.2 its sync is best-effort, and an account that yields no folders renders in neither the sidebar nor Manage Folders. Stormbox shall be permissive here rather than replicate a server's permission model, since enforcement is the server's responsibility. |
...
| FM-1.7 🟩 Implemented | A full Mailbox sync shall be authoritative only over a complete server response. A live local folder row absent from that response shall be soft-deleted so stale rows cannot remain actionable or produce `notUpdated` mutations. A rejected call carries no payload of its own — RFC 8620 §3.1.2 answers it with an `error` tuple — so any `Mailbox/get` or `Mailbox/query` that returns no payload, including one page of a paged sweep, shall fail the sync rather than shorten the authoritative list. An empty `list` remains a valid answer and shall stay authoritative, per FM-1.11. |
...
| FM-1.12 🟩 Implemented | Background metadata indexing shall cover shared accounts, not only the primary one. Each tick shall select its target folder from every Session account, ordering all primary folders ahead of all shared folders so the signed-in account never starves behind a large shared folder, and shall consider a shared folder only while it is subscribed, matching the shared-folder visibility rule in FM-6.9. A shared folder's positional coverage shall therefore advance while the client is idle, not only through foreground paging while the user has that folder open. |
...
| FM-4.9 🟩 Implemented | Pressing Escape shall close the innermost active layer first: the create dialog handles itself; then an inline editor or per-row confirmation closes; then a bulk confirmation closes; only then does the manager close. An Escape event owned by an active input-method composition shall close no layer, and an IME-owned Enter shall not save an inline rename. Backdrop activation shall close the manager. |
...
| FM-6.8 🟩 Fixed upstream | As a Stormbox interoperability requirement, a read-only user should be able to change their own server-synchronized `isSubscribed` value for a shared Mailbox without gaining rename rights. RFC 8621 requires per-user storage, while RFC 9670 still permits a server to reject some subscription changes; this was therefore an overbroad ACL coupling/interoperability bug rather than an unambiguous violation of the per-user storage MUST. Stalwart through 0.16.13 rejected every shared `Mailbox/set` update unless the user had `Acl::Modify`/`mayRename`. Stalwart 0.16.14 treats an update whose only properties are `isSubscribed`/`id` as requiring `Acl::Read` instead, so subscribing a read-only shared folder succeeds on 0.16.14 and later. The Stalwart `v1.0.0` branch predates that change and does not carry it. |
| FM-6.9 🟩 Implemented | Stormbox shall not add a local visibility fallback for read-only shared folders. It shall list the folder in Manage Folders, label it `read-only`, and disable its subscription switch when the server denies the operation. Where the server rejects the subscription and reports the folder unsubscribed — Stalwart through 0.16.13, per FM-6.8 — it remains absent from the sidebar. Server-side subscription state stays the only input to shared-folder visibility regardless of server version. |

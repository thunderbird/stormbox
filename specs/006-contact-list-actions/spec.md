...
| ID / Status | Requirement |
|:--|:--|
| CL-2.1 🟩 Done | Delete from All Contacts shall permanently destroy every selected ContactCard. Before dispatch, an alert dialog shall state that the selected contacts will be permanently deleted from all address books and that the action cannot be undone. |
| CL-2.2 🟩 Done | A scoped delete shall remove the viewed AddressBook membership when a card has another membership. If the viewed book is the card's final membership, the operation shall permanently destroy that card, as RFC 9610 forbids a surviving card with no AddressBook. |
| CL-2.3 🟩 Done | The scoped confirmation shall state that contacts present only in the current AddressBook will be permanently deleted from all address books. It may additionally show the current removal and destruction counts, but concurrent changes shall never make execution more destructive than the confirmed rule. |
...
| CL-4.3 🟩 Done | The JMAP backend shall resolve local ids, fetch fresh ContactCards and AddressBooks, revalidate rights, and issue sparse `ContactCard/set` updates/destroys in chunks bounded by the live Core limits. A mixed scoped delete may combine `update` and `destroy` in one Set call. |

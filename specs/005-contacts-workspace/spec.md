...
|:--|:--|
| CT-1.1 🟩 Done | The Contacts space shall present an address-book rail in the shell's sidebar slot, one shared virtualized directory list, and a third detail/editor column. Contact and Identity rows shall use the same list implementation and distinct detail panes inside one shared shell. |
...
| CT-1.3 🟩 Done | At desktop widths the three columns shall remain visible. The address-book rail shall be the Contacts content of the shell's sidebar slot and shall mirror the folder list's behaviour in Mail (001 R-2.9, R-10.2, R-10.10), with a visible contact or identity detail or editor pane playing the role Mail's message detail plays there: a vertically stacked list that never scrolls horizontally, sharing the slot's width, resizer, and visibility state; collapsed automatically below 1024 CSS pixels while a detail or editor pane is visible and restored when that pane closes, so list and detail remain side by side; hidden by default below 640 CSS pixels and opened as an overlay from the spaces toolbar toggle. Below 640 CSS pixels, the list and detail shall be mutually exclusive drill-in views with an accessible Back action; the inactive view shall be removed from keyboard and accessibility navigation. |

...
| CS-2.8 🟩 Done | The system shall apply the selected Identity's `replyTo`, `bcc`, `htmlSignature`, and `textSignature` defaults per RFC 8621 §6. Reply-To is applied on save/send. Bcc and signatures shall be materialized before a new/reply/forward session captures its clean seed; changing From shall replace only intact auto-added values, and a reopened server draft shall receive no inferred defaults. The complete provenance rules are `specs/005-contact-details/spec.md` CT-5.3 through CT-5.6. |

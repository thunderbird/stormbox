...
|---|---|--:|--:|--:|--:|
| 1 | Submission correctness and durability | 16 | — | — | — |
...
| CS-1.15 🟩 Done | **One send per composer.** A compose session shall have at most one send in flight, and the decision shall be taken synchronously at the first activation, before any asynchronous step — in particular before the wait for the session's in-flight autosave required by CD-6.5. Every later activation of Send while that send is unresolved, whether by click, Enter or key repeat on the focused button, the Ctrl/⌘+Enter shortcut, or a scheduled-send confirmation, shall join the send already claimed and receive its outcome; it shall never queue a second `send` mutation. The guard is a property of the session's send lane, not of the button: disabling the control follows from the sending state on the next render and is not what prevents the duplicate. From the first activation until the outcome is known, the message content — recipients, subject, body, attachments, and the send controls — shall be inert, so nothing on its way to the server can be edited or re-sent. A send may be activated again only after a prior send has settled as failed, or as unknown with no server copy per CS-1.9, where sending again is the user's decision. CS-1.6 through CS-1.9 make one mutation row deliver at most once; this requirement makes one activation burst produce at most one row. |
| CS-1.16 🟩 Done | **The screen comes back at Send.** Activating Send shall not hold the composer in the centre of the screen while the server works. The session shall leave the screen on the same tick as the activation — neither expanded nor docked — and a send toast, in the place and style of the application's other toasts, shall stand in for it: “Sending…” (or “Scheduling…”) with the CD-1.6 label and an indeterminate progress line. The toast is the session's only representation while the send is unresolved: it offers neither Restore nor Close, its content stays inert per CS-1.15, and other messages may be opened, restored, and edited meanwhile. The wait shall end at acceptance: the toast gives way to the CS-1.13 confirmation as soon as the created Email id and the submission id are confirmed and the acceptance checkpoint is durable (CS-1.3), while Sent filing, view reconciliation (CS-1.14), and draft cleanup (CD-6.7) continue in the outbox. A filing report that arrives afterwards may refine the notice but shall not reopen the composer. A send that fails or is refused shall return to the screen when no other session is expanded; otherwise the session shall dock with a “Send failed” marker and the same toast shall report the failure and offer Open until the session is restored or the notice is dismissed. The unknown-outcome rules of CS-1.9 apply unchanged. |
...
| CS-2.7 🟩 Done | The message detail view shall display Cc so the user can see the audience before replying, and Bcc whenever the stored message carries it. Only the user's own copies (drafts, scheduled, Sent) do, since the server removes the header before delivery (RFC 8621 §7.5); on those copies Bcc is the only record of who was blind-copied. |
...
| CS-3.8 🟩 Done | When a non-empty suggestion list opens, its first option shall already be highlighted and exposed through `aria-activedescendant`; Arrow Down shall therefore advance to the next option rather than selecting the first one. Enter shall accept the highlighted suggestion and remain in the field; Tab shall accept it and move to the next field. When no suggestion is available, either key shall commit valid typed input or report a precise parse error. The recipient control shall not act on keys owned by an active input-method composition. |
...
| CS-3.17 🟩 Done | Tab shall move through the compose dialog in DOM order: From, To, the Cc/Bcc toggles or fields, Subject, body, footer controls, then the window controls, wrapping at either end. A closed dropdown's trigger is a stop like any other; Tab from it shall go to the next control, not to the first control in the dialog. The formatting toolbar shall follow the WAI-ARIA toolbar pattern: a single Tab stop between Subject and the body, with Arrow Left/Right (reversed under `direction: rtl`), Home and End moving between its top-level controls and the most recently focused control keeping the stop, so Tab back into the toolbar returns to it. Items inside the toolbar's dropdown menus are not toolbar stops. |
...
| ID / Status | Requirement |
|:--|:--|
| CS-5.1 🟩 Done | Recipient delivery shall be covered end to end: To, Cc, and Bcc delivered to separate accounts, with Bcc absent from the delivered headers and present on the stored Sent copy and in its detail view. |

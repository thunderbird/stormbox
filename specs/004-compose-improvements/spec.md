...
| CS-3.17 🟩 Done | Tab shall move through the compose dialog in DOM order: From, To, the Cc/Bcc toggles or fields, Subject, body, footer controls, then the window controls, wrapping at either end. A closed dropdown's trigger is a stop like any other; Tab from it shall go to the next control, not to the first control in the dialog. The formatting toolbar is not a Tab stop — none of its buttons, menus or colour inputs take sequential focus — so Tab from Subject lands in the body; formatting stays reachable by pointer and by the shortcuts each button advertises through `aria-keyshortcuts`. |

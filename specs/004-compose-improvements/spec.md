...
|---|---|--:|--:|--:|--:|
| 1 | Submission correctness and durability | 13 | 1 | — | — |
...
Four requirements remain partial. CS-1.14 persists only the Email counters
returned by its post-send `Mailbox/get`, not the thread counters. CS-2.3
accepts an unpaired UTF-16 surrogate in an unquoted display-name atom.
CS-4.2 has the accepted unknown-address-book residue described in its row.
For CS-5.4, every package that changes server and cache state has shipped
Playwright coverage on both engines, but one assertion of the three the row
asks for is missing: `compose-reply-audience.spec.js` proves the reply
audience and threading headers through the UI and through JMAP on the
...
| CS-1.14 🟨 Partial | Sent-folder reconciliation after a send shall derive list position and total from the server rather than assuming them, and shall reconcile the affected Mailbox counters too. Each open Sent view now takes the new message's position and authoritative total from its own `Email/query`, marking an unanswered view stale. The accompanying `Mailbox/get` requests `totalEmails`, `unreadEmails`, `totalThreads`, and `unreadThreads`, but the local update persists only `total_emails` and `unread_emails`; the two thread counters can therefore remain stale. |
...
| CS-2.8 🟩 Done | The system shall apply the selected Identity's `replyTo` default on send, per RFC 8621 §6. Identity-level automatic Bcc is outside the product scope: Stormbox shall neither request nor persist that default, and only recipients explicitly present in the draft shall be Bcc'd. |
...
| CS-3.8 🟩 Done | Explicit user input shall take precedence over suggestions. Enter shall accept a suggestion only while one is highlighted; otherwise it shall commit valid typed input or report a precise parse error. The recipient control shall not act on keys owned by an active input-method composition. |
...
| CS-4.5 🟩 Done | `Identity/get` shall be applied as an authoritative snapshot, including the empty-list case, so a removed identity cannot linger in the From picker. The `replyTo` property shall be persisted. |

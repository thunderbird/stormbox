...
| CS-1.16 🟩 Done | **The screen comes back at Send.** Activating Send shall not hold the composer in the centre of the screen while the server works. The session shall minimize to its docked bar on the same tick as the activation, with the bar showing “Sending…” (or “Scheduling…”) and offering Restore but not Close; other messages may be opened, restored, and edited meanwhile, and a restored sending session remains inert per CS-1.15. The wait shall end at acceptance: the composer closes and the CS-1.13 confirmation appears as soon as the created Email id and the submission id are confirmed and the acceptance checkpoint is durable (CS-1.3), while Sent filing, view reconciliation (CS-1.14), and draft cleanup (CD-6.7) continue in the outbox. A filing report that arrives afterwards may refine the notice but shall not reopen the composer. A send that fails or is refused shall be restored from the dock when no other session is expanded; otherwise its bar shall show “Send failed” with a marker until the user restores it. The unknown-outcome rules of CS-1.9 apply unchanged from the dock. |
